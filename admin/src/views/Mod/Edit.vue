 ...
 * @LastEditTime: 2021-08-29 00:01:40
-->
<template>
  <div>
    <h1>{{ id ? "编辑遗物" : "新建遗物" }}</h1>
      ...
      <el-form ref="form" :model="form" label-width="100px" label-position="top">
        ...
        <el-row :gutter="10">
          <el-form-item label="是否入库">
            <el-radio v-model="form.stock" label="1">未入库</el-radio>
            <el-radio v-model="form.stock" label="2">已入库</el-radio>
          </el-form-item>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    // 找出选中类型的名称
    chooseType(e){
      this.tempType = this.remainTypeList.filter(item=> (item._id == e))[0].name
    },
    // 数据回显
    async getData() {
      const res = await this.$api.getRemain(this.id, 'remain');
      this.form.name = res.data.name.replace(/[\u4e00-\u9fa5]/g, "")
      this.form.type = res.data.type.name
      this.form.cu1 = res.data.cu1.name
      this.form.cu2 = res.data.cu2.name
      this.form.cu3 = res.data.cu3.name
      this.form.ag1 = res.data.ag1.name
      this.form.ag2 = res.data.ag2.name
      this.form.au = res.data.au.name
      this.form.stock = res.data.stock
      this.form.getways = res.data.getways
      this.tempForm = res.data
    },
    async onSubmit() {
      if (this.id) {
        // 先对数据进行处理
        let keys = ['type', 'cu1', 'cu2', 'cu3', 'ag1', 'ag2', 'au']
        keys.forEach(item=>{
          if(this.form[item] == this.tempForm[item].name){
            this.form[item] = this.tempForm[item]._id
          }
        })
        this.form.name = this.form.name.replace(/[\u4e00-\u9fa5]/g, "")
        this.form.name = this.remainTypeList.filter(item=> (item._id == this.form.type))[0].name + this.form.name + '遗物'
        ...
        this.form.name = this.tempType + this.form.name + '遗物'
