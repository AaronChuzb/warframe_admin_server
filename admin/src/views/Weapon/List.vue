  ...
  <div>
    ...
    <el-table :data="remains" style="margin-top: 1vw">
      ...
      <el-table-column align="center" fixed="right" label="操作" width="180">
        <template slot-scope="scope">
          <el-button type="primary" size="small" @click="$router.push(`/remain/edit/${scope.row._id}`)">编辑</el-button>
  ...
  methods: {
    ...
    async remove(row) {
      this.$confirm(`是否删除"${row.name}"`, "警告", {
        ...
        await this.$api.deleteRemain(row._id)
        this.$message({
          type: "success",
          message: "删除成功!",
        });
